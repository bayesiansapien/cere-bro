...
- **Spotify published the engineering detail behind cutting Claude Code token usage 90%**, and it is a routing result: keep the frontier model for reasoning, intercept expensive file reads with PreToolUse hooks, and route them to Gemini 2.5 Flash ([Spotify Engineering](https://engineering.atspotify.com/2026/9/portal-by-spotify-cut-my-claude-code-token-usage-by-90)).
- **The cost figures in that post are the reason it matters**: a quarter of engineering leaders already spend **$200 to $500 per developer per month** on tokens, some are past **$2,000**, and AI coding costs are projected to exceed the average developer's salary by 2028 (same source).
- **SigMap** gives coding agents verifiable repository context with no LLM calls, embeddings or vector database, placing the correct file in its top five **81.1% of the time against 44% for single-shot grep while cutting tokens 96.8%** across 21 repositories (via [@agenticgirl](https://x.com/agenticgirl)).
- **OpenBMB open-sourced MiniCPM5-2B**, a 2B model averaging 53.9 across 34 coding, math, long-context, tool-calling and agent benchmarks, releasing data, training recipe and RL stack alongside the weights ([HuggingFace](https://huggingface.co/openbmb)).
- **Tencent's FlowBalance** attacks self-guidance collapse in reasoning models by tying dense self-feedback to the verifier's group advantage: keep the guidance on positive advantage, reverse it on negative, shut it off when the rollout shows no preference ([arXiv 2609.03241](https://arxiv.org/abs/2609.03241)).
