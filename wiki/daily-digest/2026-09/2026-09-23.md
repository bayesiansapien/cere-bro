...
> Six independent results today say the same thing in six vocabularies: the technique was never the interesting variable, the **allocation** was. How to split bits across attention heads, precision across experts, memory slots against evictions, I/O against arithmetic, and dollars across a memory-majority semiconductor market. And the day's other half is a measurement crisis reaching its fourth subfield in eight days, with the counter-move arriving alongside it.
...
**Links:** [KV-COBRA paper](https://arxiv.org/abs/2609.24298) · [Colla-Q paper](https://arxiv.org/abs/2609.18131) · [KV-COBRA summary](../../inference-efficiency/2026-09-23-kv-cobra-bit-rank-allocation.md) · [Colla-Q summary](../../inference-efficiency/2026-09-19-colla-q-moe-quantization-minimax.md)
...
→ [KV-COBRA](../../inference-efficiency/2026-09-23-kv-cobra-bit-rank-allocation.md) · [Colla-Q](../../inference-efficiency/2026-09-19-colla-q-moe-quantization-minimax.md)
...
### Constrained decoding got a thousand times faster, and it quietly solved the problem the decision models are sold on

> Grammar-guided generation used to cost 20 to 50 milliseconds per token, so nobody ran it at scale. It now costs under 40 microseconds. Schema-valid output has been free for a while and the field has not updated.
...
**What is it about?**
Constrained decoding, also called grammar-guided generation or structured outputs, compiles a schema, regular expression or grammar into a finite state machine, and at every generation step sets the logits of syntactically illegal tokens to negative infinity before sampling. The output is then guaranteed to parse. The chapter's argument is a status update: this used to be unaffordable, it is now free, and the field has not priced that in.

**What problem does it solve?**
The legacy answer to malformed model output is a retry loop. Catch the JSON decode error in middleware, feed the traceback back into context, try again three to five times. That carries a latency penalty, with P99 going from 400 milliseconds to over 3.5 seconds on a single schema failure, a token penalty from resubmitting conversation history plus error traces, and no safety guarantee at all, since at non-zero temperature there is still a non-zero chance of violation on the fifth attempt.

**What is the core novelty?**
Not a new method, a measurement and a cost nobody counts. GPU-accelerated grammar compilers, SGLang's XGrammar, Outlines' trie-FSM indexers and native token-masking kernels in llama.cpp and vLLM V1, execute grammar validation in **under 40 microseconds per step**, down from 20 to 50 milliseconds on CPU. That is roughly a thousandfold reduction, which puts schema enforcement below the noise floor of a decode step.

**Key takeaways**
- **The uncounted cost is KV cache thrashing.** Malformed tokens pollute prefix and KV allocations with discarded context fragments, forcing premature cache evictions **for concurrent users**. That is a multi-tenant externality rather than a cost borne by the failing request, and no standard observability stack attributes it.
- So constrained decoding is a serving-efficiency feature, not only a reliability one, which is a reframing this wiki did not have.
- **It explains the decision-model boom mechanically.** The typed-output guarantee that category is sold on is available in vLLM today with no second model, so what a decision model actually adds is the single forward pass and the probability, not the format safety.
- The chapter is honest about its own limit, echoing the author's 09-21 line: type safety prevents malformed output, not incorrect judgment, and a schema-valid mistake refunds the wrong customer just as fast.
...
**Industrial implication**
Narrow and worth acting on this week: if your agent stack still runs a parse-retry loop for structured tool calls, the cost is not only your own latency and tokens, it is cache evictions for every co-tenant on the node, and the fix is a serving-engine flag rather than a project. Broader: a large fraction of the 160-plus decision-model integrations shipped this month are gates that could equally be a constrained-decoding call on a model already in the stack, with no second vendor, no second API key and no unknown calibration. Today's vLLM merge of structured generation for DiffusionGemma is the two paths converging in one codebase.
...
> The highest-rated paper on either Kurate board this week is two interval endpoints and a proof. HuggingFace did not notice it.
...
**What is it about?**
Linear recurrent networks built on the delta rule, an update that corrects a stored memory by a small low-rank amount each step, are the main efficient alternative to attention: constant memory, linear time, and no KV cache that grows with context. Their known weakness is expressivity, specifically in tracking state over long sequences.

**What problem does it solve?**
Prior work showed that composing two delta-rule transitions in one recurrent update lets a model represent a 2D rotation, which unlocks a large class of state-tracking problems. The catch is that composing two transitions raises the rank and the cost of every update, giving back the efficiency that made these models interesting in the first place.

**What is the core novelty?**
Kimi Delta Attention already contains the missing ingredient. Its channel-wise gate can supply a **reflection**, and one delta-rule transformation composed with one reflection is a rotation. The only change required is widening two parameter ranges that already exist: letting the gate take values in [-1, 1] and the delta coefficient in [0, 2]. Both extensions existed separately in prior work. The contribution is noticing that combining them produces the rotation for free.

**Key takeaways**
- An exact characterization rather than a lower bound: **every orthogonal diagonal-plus-rank-one matrix is a CKDA transition matrix.** One layer can track every finite group isomorphic to a subgroup of SO(3).
...
- Transitions stay non-expansive and diagonal-plus-rank-one, so stability and the cheap update both survive. Expressivity gains in recurrent architectures usually cost stability. This one does not.
...
**Gaps in the study**
The honest caveat is in the paper's own results: in language modelling CKDA "obtains similar results to a KDA baseline." The expressivity gain shows up on state-tracking diagnostics and on length extrapolation, not on the loss curve, and "promising scaling behavior" without a scaling curve at frontier size is precisely where this literature has over-promised before. The diagnostic tasks were chosen because they are known to require the capability being added, which is right for a theory paper and wrong for a deployment argument. And the result is specific to KDA, so how much transfers to Mamba-family or gated-linear-attention variants is unaddressed.

**Industrial implication**
Read this against the rest of today's digest. Flash-dLLM, KV-COBRA, KVMEM and ARM are four attempts to make a growing KV cache affordable, and the semiconductor numbers say the market is paying for that growth in memory. The linear-RNN line's answer is to not have a growing cache at all, and given the 09-21 arithmetic that one million-token stream needs 137.44 GB on a 70B model, that is not a niche position. The reason it has been dismissed is the expressivity gap. **Today that gap got narrower and, more usefully, exactly characterized, which is the first time this wiki can state what the constant-memory alternative can and cannot represent rather than guessing.**
...
- **Anthropic released Claude Opus 5.5 at $4/$20 per million tokens**, a 20% cut, claiming Fable 5.1 performance at roughly 40% lower running cost ([The Decoder](https://the-decoder.com/claude-opus-5-5-matches-fable-5-1-at-40-percent-lower-cost-as-anthropic-promises-to-fix-claudish-writing/)).
- **OpenAI answered within the hour with GPT-6 Sol and Luna at half their predecessors' prices**, Luna landing at $0.10/$0.50 with $0.01 cached input ([The Decoder](https://the-decoder.com/openais-gpt-6-sol-and-luna-cut-prices-in-half-but-barely-move-the-needle-on-performance/)).
- **Independent analyses found little actual capability gain in the new GPT-6 pair**, making this a price action rather than a capability release ([Simon Willison](https://simonwillison.net/2026/Sep/22/opus-and-sol-and-luna/)).
- **Claude Opus 5.5 at max thinking level failed to return a response at all** on Willison's standard SVG test, twice, exhausting its 128k output limit while still reasoning, at $2.56 and 20 minutes per failure ([Willison](https://simonwillison.net/2026/Sep/22/opus-and-sol-and-luna/)).
- **HuggingFace Transformers now runs llama.cpp quants directly**, collapsing the split between the research and practitioner quantization stacks ([HF blog](https://huggingface.co/blog/transformers-llama-cpp-quants)).
...
- **China's internet regulator opened investigations into DeepSeek and Moonshot AI** after Anthropic's 154-page report alleged both routed sensitive user data to Claude ([The Information](https://www.theinformation.com/briefings/china-probes-deepseek-moonshot-data-leaks-anthropic)).
- **Alibaba unveiled the Zhenwu V900 AI chip**, claiming three times its predecessor's performance, alongside a data-centre expansion plan ([The Information](https://www.theinformation.com/briefings/alibaba-unveils-ai-chip-data-center-expansion)).
- **Z.ai open-sourced its ZCode coding tool** after developers found their data was being uploaded to cloud servers without consent ([The Information](https://www.theinformation.com/briefings/z-ai-open-sources-zcode-data-uploading-scandal)).
...
- **Meta's Muse passed 500,000 users in its first week** with 250,000 daily actives and over 2 million prompts ([The Information](https://www.theinformation.com/articles/meta-muse-surpassed-500000-users-first-week)).
- **Gary Marcus published the sharpest counterweight to that number**, noting Meta tried the same thing in 2015 as Project M, reached only 10,000 users, turned out to be serving over 70% of requests with humans behind the scenes, and cancelled it in January 2018 ([Marcus on AI](https://garymarcus.substack.com/p/the-secret-behind-metas-muse)).
- **Microsoft detailed its Windows AI plan for the first time**: native agent identity, an OS-level MCP server registry, an OS-agnostic tool isolation mechanism, and WindowsML as a hardware abstraction layer for local models across GPU, NPU and CPU, with a goal of shipping small language models without requiring an NPU ([Pragmatic Engineer](https://newsletter.pragmaticengineer.com/p/how-will-ai-change-operating-systems-part-2-windows)).
- **Gradient Flow made the week's sharpest data-infrastructure argument**: humans sitting between a query and an action tolerated stale or ambiguous data, agents do not, and a semantic layer that answers "I cannot answer" beats raw SQL returning a plausible wrong number ([Gradient Flow](https://gradientflow.substack.com/p/why-your-data-stack-is-about-to-get-less-forgiving)).
- **Interconnects published a long debate with Epoch AI's JS Denain** on recursive self-improvement and the US-China gap, whose most concrete public number is OpenAI's disclosed 2x-per-month growth in Codex spending by its own researchers ([Interconnects](https://www.interconnects.ai/p/debating-rsi-the-us-china-gap-and)).
...
- **CoreWeave completed a $4.2 billion convertible bond offering**, upsized from the $3 billion it targeted the week before ([The Information](https://www.theinformation.com/briefings/coreweave-completes-4-2-billion-convertible-offering)).
- **Ande raised over $52 million** across seed and Series A from Lightspeed, Redpoint, Duration, Sierra and Bain Capital Ventures for corporate entertainment-booking agents, with Cloudflare, Salesforce and McGraw Hill as customers ([The Information](https://www.theinformation.com/briefings/entertainment-booking-startup-ande-raises-52-million-lightspeed-redpoint)).
- **Binance invested $100 million in Circle** alongside a five-year USDC distribution agreement ([The Information](https://www.theinformation.com/briefings/binance-invests-100-million-circle)).
- **Wall Street's push for Nvidia GPU rental futures stalled**, with the CFTC extending its review past CME's hoped-for early-October launch ([The Information](https://www.theinformation.com/briefings/wall-street-gpu-futures-push-stalls-cftc)).
- **AI providers took 8% of customer software spending in the year to August, up from 1.4%**, per Zip procurement data across dozens of customers averaging 2,400 employees ([The Information](https://www.theinformation.com/briefings/new-data-anthropic-openai-upstarts-eating-software-budgets)).
- **Microsoft authorized 30% to 50% Copilot discounts** for large seat commitments, and Amazon, Figma and Workday are all discounting AI to hold customers against Anthropic and OpenAI ([The Information](https://www.theinformation.com/articles/microsoft-boost-copilot-discounts-launches-ai-super-app)).
- **Barrick Gold's North American unit signed a five-year AI partnership with Avathon** ahead of a planned year-end IPO of a minority stake ([The Information](https://www.theinformation.com/briefings/barrick-gold-inks-five-year-ai-deal-ipo)).
- **Roughly $130 billion of US data-centre projects were blocked or delayed in Q1 2026**, 71% of surveyed Americans oppose one near them, and Texas has frozen new grid-connected projects pending impact studies ([The Information](https://www.theinformation.com/articles/why-americas-ai-dream-failing-launch)).
---
