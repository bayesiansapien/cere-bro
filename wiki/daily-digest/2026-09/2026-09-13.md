...
**Two groups arrived independently on the same day at the claim that models cannot sequence prerequisites, and only one of them proposed fixing it outside the model.** MT-INFOSEEK found that on tasks with ordered dependencies, an incorrect query order lowers final accuracy even when the model eventually acquires everything it needs, and that models under-predict how much information is missing about four times as often as they over-predict it. State-Path Tool Menus found that ranking tools by request relevance reliably surfaces the final action while omitting the tools that produce its inputs, and fixed it by moving the ordering into the retrieval layer, lifting ToolBench online success 0.737 to 0.898 **with the agent unchanged**. One paper measures the deficit and one routes around it. **The composition, evaluating dependency-aware menus on MT-INFOSEEK's ordered-dependency tasks, is the cheapest useful experiment either paper enables**, and it matters commercially because every requirements-gathering agent in production today scores its final answer and therefore cannot see this failure at all.
