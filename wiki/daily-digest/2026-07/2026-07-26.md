...
- **Tim Cook is lobbying Washington to let Apple buy memory from China's CXMT and YMTC** for devices sold outside the US, both Pentagon-designated Chinese military companies, YMTC on the Entity List ([@MarioNawfal](https://x.com/MarioNawfal/status/2081035654365102181)).
- **Cook is simultaneously accusing Micron of gouging at 80% margins** while calling the DRAM shortage a "100-year flood," which are not compatible complaints ([@MarioNawfal](https://x.com/MarioNawfal/status/2081035654365102181)).
- **Kimi K3 ships open weights Monday**, alongside a run of competitive releases from Thinking Machines, Poolside, Motif, and Upstage ([@eliebakouch](https://x.com/eliebakouch/status/2080898494710100042)).
- **OpenAI has reportedly signed the open-weights letter** it declined two days ago, per chatter on X with no primary confirmation yet; treat as unconfirmed ([@thogge via @ns123abc](https://x.com/ns123abc/status/2081145123761459607)).
- **Robert Scoble is touring Starcloud in Seattle** on orbital datacenters, a thread that keeps recurring as the terrestrial power-and-memory constraint tightens ([@Scobleizer](https://x.com/Scobleizer/status/2081113992395890737)).
...
- **Apple is seeking a regulatory carve-out rather than a supply contract**, which is what a buyer does when the constraint is allocation rather than price ([@MarioNawfal](https://x.com/MarioNawfal/status/2081035654365102181)).
...
- No new AI funding rounds or acquisitions appeared in today's sources. Saturday-into-Sunday is consistently the thinnest window of the week for this category, and yesterday's cluster (NVIDIA and SK Group at $500B, Stripe's reported ~$10B bid for OpenRouter) has no follow-on reporting yet.
...
- **Opus 5's zero breaks in one of two ways within 90 days, and both are already predictable.** First, the capability inversion: Sonnet 5 alone scores 0.93% against Opus 5 alone at 3.7% on the same evaluation with no mechanism offered, so watch for an independent evaluation plotting injection success against capability across three or more sizes in one family. If that relationship is monotone, injection resistance must become a separately optimized objective and the default of "use the strongest model for the agent reading untrusted content" is wrong. Second, the persistence gap: both Auto Mode layers are per-step and [ClawTrojan (06-01)](../../responsible-ai/2026-06-01-clawtrojan-persistent-control-trojan.md) hit 95.5% attack success with multi-step trojans precisely because per-step defenses miss the earlier benign-looking write, so watch for a published plant-now-trigger-later attack across sessions, or Anthropic extending the evaluation to persistent memory and reporting a number above zero.
- **The administration denies Apple's CXMT and YMTC carve-out within 90 days, keeping memory contention fully priced into HBM.** Apple is asking for a regulatory exemption rather than negotiating a supply contract, which is what a buyer does when the binding constraint is allocation rather than price, and Cook's own "100-year flood" framing concedes the shortage is structural rather than a Micron margin story. Signal: any Commerce or BIS determination on Apple sourcing from entity-listed Chinese memory makers for non-US devices. A grant would be the first crack in the allocation regime this wiki's [memory-hierarchy](../../hardware/memory-hierarchy.md) page has projected running into 2030; a denial or continued silence means consumer device demand keeps bidding against datacenter HBM for the same wafer starts, which is a floor under inference cost that essentially nobody models.
