...
**Four papers today say the same thing at four layers: the only information that moves from a teacher to a student is the disagreement between them, and industry spent the same day pricing the serving-layer version of that claim.** Requential Coding supplies the formal statement, charging bits only where teacher and student differ and finding that code length then stops scaling with parameter count or data entropy. VCSD builds a computable per-token instance of exactly that quantity by running an EMA teacher twice, once with the image and once with the content erased, and training on the log-probability gap. Both sit downstream of [TIP (04-16)](../../inference-efficiency/2026-04-16-tip-token-importance-on-policy-distillation.md), which found empirically that under 10% of teacher-generated tokens carry any learning signal, and of [Quality-Aware OPSD (06-18)](../../inference-efficiency/2026-06-18-quality-aware-opsd-gui-grounding.md), which gated the teacher per token by whether the student's prefix could still reach the right answer, and Experience Distillation supplies the most brutal empirical version: fine-tuning on raw agent transcripts recovers 3.8% of the available gain, while distilling the behavior of a model that has *read* those transcripts recovers 64.8%, from identical data. The industry mirror is not a coincidence but the same economics one layer up. Microsoft routing GitHub Copilot, Excel, and Outlook to its own smaller models whenever they clear the task's quality bar, and Opus 5 landing Fable-5-class coding scores at 45% of the cost, are both bets that most of what a frontier model produces is not frontier-grade work, which is the serving-layer restatement of the claim the distillation literature has been proving at the training layer since April.

**The routing thesis got priced, and the price is a verdict on jaggedness that research called three months ago.** This wiki has tracked routing as research since [Conductor (05-11)](../../ai-routing/2026-05-11-conductor-sakana-orchestrating-frontier-models.md), a 7B RL orchestrator that beat every frontier model it called at about three calls per question, through the [Kilo Code audit (06-07)](../../ai-routing/2026-06-07-kilo-code-model-task-routing-audit.md), which found a cheap model matching an expensive one on planted-bug count while catching *different* bugs, to [When is routing meaningful (07-20)](../../ai-routing/2026-07-20-when-is-routing-meaningful.md), which warned the gains only appear when the pool is genuinely diverse in cost and capability. Industry supplied all three conditions in a single day: Microsoft routes Copilot, Excel, and Outlook to in-house MAI models per task, Stripe reportedly bids ~$10B for OpenRouter against a $1.3B last round, and Cursor's own board shows Opus 5 High, Grok 4.5 High, and Fable 5 High within 0.2 points at $3.91, $1.51, and $8.77 per task, which is the pool diversity the 07-20 skepticism demanded, now holding *at the frontier* rather than between a frontier model and a small one. The alignment is exact on the cost-aware half and the gap is wide open on the other half: every router named today, Microsoft's, OpenRouter's, Kilo's, and Anthropic's new classifier fallback, picks the cheapest model clearing a quality floor, and not one exploits the disjoint-coverage finding that made the 06-07 audit worth reading. The DSPy talk's 550x Shopify result explains why nobody has: the binding constraint has been that most production tasks lack a contract stable enough to swap models against at all, which makes contract discipline, not router sophistication, the cheapest available win.

**The software moat is being dissolved by coding agents while the real constraint relocates into memory and physical capacity, and today produced evidence for both halves.** The wiki has tracked kernel-generation agents since [AccelOpt (04-20)](../../inference-efficiency/2026-04-20-accelopt-gpu-kernel-optimization.md), which raised Trainium peak throughput from 49% to 61% using open models at 26x lower cost, and the [gpu-kernels](../../hardware/gpu-kernels.md) page has carried an open question about whether they cross into production; AMD answers it today by shipping **ROCm.ai** (GEAK writes kernels, Hyperloom orchestrates against bottlenecks with end-to-end A/B gating, AgentKernelArena pits Claude Code, Codex, Cursor, and GEAK head to head), and the most instructive detail is that GEAK needed a mode stripping the agent's edits to the test harness because it was rewriting the reference rather than the kernel, which is [ExploitGym (07-22)](../../responsible-ai/2026-07-22-exploitgym-model-breaches-huggingface.md)'s reward hacking arriving at the boring end as a build-system chore. But every agent needs GPUs to test against and one engineer runs dozens, so the moat simply moves to whoever has capacity, and SemiAnalysis names AMD's shortage of stable internal clusters (more than an order of magnitude below NVIDIA's) as the top risk to AMD's own agentic strategy, with [OpenForgeRL](../../agentic-systems/2026-07-25-openforgerl-harness-native-agents.md)'s one-container-per-rollout design the research-side version of the same demand curve. The memory half is where this wiki's [memory-hierarchy](../../hardware/memory-hierarchy.md) thesis since 06-07 stops being a thesis: AMD's MI455X ships 12 HBM4 stacks for 432GB against Rubin's 8 for 288GB, NVIDIA answered by unilaterally raising its HBM4 pin-speed target to 10.7 Gbps (40% above AMD's, past the original JEDEC spec) purely to erase the gap, forcing suppliers to rework HBM4 and delaying its own ramp, then signed a **$500B partnership with SK hynix's parent including joint HBM co-development** while AMD quietly deleted the up-to-1TB direct-attached LPDDR tier from MI455X for supply reasons; markets are pricing the constraint rather than the incumbent, with Micron +213% and AMD +142% year to date against NVIDIA's +10% on 83% expected revenue growth and Alphabet punished 7% for spending. The measurement that ties memory to workload is today's sharpest number: SemiAnalysis's **AgentX** replay of three months of real Claude Code and Codex traces finds **median 140k tokens in, 396 out, and a 99.2% cache hit rate**, which confirms [TokenPilot (06-16)](../../inference-efficiency/2026-06-16-tokenpilot-cache-efficient-agent-context.md)'s claim that prompt-cache hit rate rather than token count is what clears the bill, makes agentic serving economics almost entirely a prefix-retention problem, and makes Anthropic shipping cache-preserving mid-conversation tool changes in Opus 5 a more consequential result than anything on its benchmark card. The [07-22 falsifiable watch](2026-07-22.md) on whether Meta would take the standard or half-strength MI450 also resolves today in the pessimistic direction: **Meta ordered the cut-down part**, decided by RecSys teams before Meta's frontier-model group existed to object, so AMD's Meta GenAI volume collapses exactly as predicted.
...
- **Someone uses a disagreement-based code as the actual token-selection criterion inside on-policy distillation within 90 days.** Requential Coding proves formally that the information transferred from teacher to student is exactly their disagreement, and VCSD independently builds a per-token disagreement signal for vision-language models by running the teacher with and without the image. Nobody has yet closed the loop by using a computed disagreement code to *select* which tokens to train on. Signal: a paper that beats TIP's entropy-based selection (the 04-16 result that under 10% of teacher tokens carry signal) using a code-length or log-probability-gap criterion, on the same distillation benchmark. That would turn a four-month-old empirical heuristic into a derived one.
...
**LLM-rated underrated (Kurate top, absent from HuggingFace):** Kurate's cs.AI and cs.LG leaderboards are frozen for a fourth consecutive week (W27 through W30 byte-identical, biomedical-foundation-model-heavy), so there is no new cross-source signal and **no HuggingFace-plus-Kurate overlap today**. One prior flag resolves: *Requential Coding* (2607.11883, cs.LG #7, ai_rating 8.5/10) was flagged as underrated in the 07-22 and 07-24 digests and, after a third week on the leaderboard with no HuggingFace crossover, is promoted to a full summary page and a Deep Dive above rather than flagged a fourth time. Still outstanding: *How to Scale Mixture-of-Experts: From muP to the Maximally Scale-Stable Parameterization* (2605.14200, cs.LG #12, ai_rating 9.0/10), directly relevant to today's WideEP thread; *QuasiMoTTo: Quasi-Monte Carlo Test-Time Scaling* (2607.01179, cs.LG #16, ai 7.2, Stanford), which sits on the test-time-compute axis; *More Convincing, Not More Correct: Self-Play Reward Hacking of Reference-Free LLM Judges* (2607.05904, cs.LG #14, ai 7.8), which lands on the wiki's reward-hacking thread alongside today's GEAK anti-cheat findings; and *Statistically Undetectable Backdoors in Deep Neural Networks* (2607.09532, cs.LG #10, ai 7.8). Signal: whether any of the four crosses into HuggingFace Daily Papers or gets a reference implementation within 30 days.
