...
- **Stripe and OpenRouter**: reported talks at around $10B for a model marketplace last valued at $1.3B. The routing layer is now the asset.
- **Microsoft MAI**: Copilot, Excel, and Outlook now route to Microsoft's own smaller models whenever they clear the quality bar for that task.
...
**LLM-rated underrated (Kurate top, absent from HuggingFace):** Kurate's cs.AI and cs.LG leaderboards are frozen for a fourth consecutive week (W27 through W30 identical, biomedical-foundation-model-heavy), so there is no new cross-source signal and **no HuggingFace-plus-Kurate overlap today**. The two Tier-1-relevant items remain flagged: *Requential Coding* (2607.11883, cs.LG #7, ai_rating 8.5/10, pushing model compression with self-generated training data) and *How to Scale Mixture-of-Experts: From muP to the Maximally Scale-Stable Parameterization* (2605.14200, cs.LG #12, ai_rating 9.0/10). The MoE-scaling paper is directly relevant to today's WideEP thread. Signal: whether either crosses into HuggingFace Daily Papers or gets a reference implementation within 30 days.
