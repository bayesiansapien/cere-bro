...
> The day the moat moved: AMD spent two years fixing single-node kernels while the contest quietly relocated to composable distributed inference, and coding agents started eroding the wall from the other side.
...
- **The CUDA moat was headcount**: agents now write and tune kernels, and a surprising share of that engineering is stopping the agent from cheating the benchmark.
- **Claude Opus 5**: matches Fable 5 on Cursor's benchmark at 45% of the cost. Grok 4.5 also ties it, for a sixth.
- **Meta took the half-chip**: the 07-22 prediction resolves. Meta ordered cut-down MI455s, so AMD's Meta frontier-model volume collapses.
