...
- **Kimi K3 tops the frontend Code Arena but collapses on hard math**: The Decoder reports Kimi K3 is the first Chinese model to lead Code Arena: Frontend, beating Fable 5 and GPT-5.6 Sol by a wide margin, yet scores only ~39% on FrontierMath Tier 4 where OpenAI and Anthropic hit ~90%. The "open weights at the frontier" claim is real but jagged: strong on frontend code, far behind on complex reasoning ([The Decoder](https://the-decoder.com/moonshots-kimi-k3-outperforms-fable-5-in-frontend-code/)).
- **Alibaba answers with Qwen 3.8 (2.4T)**: an open-weight multimodal model the Qwen team says is "second only to Fable 5." Two Chinese open-weight frontier releases in a week ([The Decoder](https://the-decoder.com/alibabas-qwen-takes-on-kimi-k3-with-open-weight-qwen-3-8/)).
- **Google DeepMind's GenCeption: video generators already contain world models**: repurposes a video generator for depth estimation and segmentation, matching state-of-the-art with far less (mostly synthetic) training data, feeding the "video models are universal world models" debate that From Pixels to States also touches ([The Decoder](https://the-decoder.com/google-deepmind-argues-video-generators-already-contain-the-world-models/)).
- **Alphabet + SpaceX drive record US equity sales**: 2026 US stock sales on track to pass the 2021 record, powered by mega-offerings ([The Information](https://www.theinformation.com/articles/alphabet-spacex-spur-record-u-s-equity-sales)).
