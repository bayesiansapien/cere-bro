...
> The first fully-open training recipe for multimodal deep-search agents that the wiki has tracked. Tencent Hunyuan + UCLA + CUHK + HKU. Targets the data-and-environments-as-moat critique.

**Source:** HuggingFace Daily Papers (2605.05185), enriched with alphaxiv overview
**Links:** [Paper](https://arxiv.org/abs/2605.05185) · [Wiki](../../agentic-systems/2026-05-08-opensearch-vl-multimodal-search.md)
**Tier:** 2 — agentic-systems / multimodal

The contribution is structural rather than algorithmic. The paper identifies three concrete bottlenecks in current multimodal-search-agent literature. Shallow perception-level queries, neglect of robust visual pre-processing for imperfect inputs (blurred, low-resolution, skewed images), and difficulty applying RL to long-horizon tool-use settings where cascading tool failures invalidate entire rollouts. The contribution is an open data + environment + RL-recipe stack that addresses all three. The implicit claim is that the gap between proprietary frontier multimodal agents and open recipes is not algorithmic, it is data-and-infrastructure. If true, the open ecosystem catches up faster than the closed-vs-open trajectory in pure LLMs would predict.

**Why it matters:** This is the open-recipe analogue of the build-not-buy mentality Lambert documents in his China piece. The Chinese-affiliated authors (Tencent Hunyuan + CUHK + HKU) plus the data-and-environments-as-moat framing fit Lambert's pattern that "the data industry is far less developed" in China and labs build their own. OpenSearch-VL is the open-source instantiation of the same pattern that ResRL represents in RLVR.

**Research angle:** The cascading-tool-failure RL problem is the most interesting unsolved piece. How does OpenSearch-VL's RL algorithm handle partial-credit assignment when 4 of 7 tool calls succeed? The paper claims an improvement over standard agentic RL, but the head-to-head against [t2po](../../agentic-systems/2026-05-05-t2po-uncertainty-multi-turn-rl.md) (05-05) on its own benchmarks would settle which approach generalizes.

→ [Full summary](../../agentic-systems/2026-05-08-opensearch-vl-multimodal-search.md)
