...
> Some attention to your tears. Read through the optimization lens, today is the day the cost axis turns around and bites: the biggest number on the board is a **5 to 8% increase** in production compute caused by AI-written code, measured across 3.52 million changes, which is larger than most of what the entire inference-efficiency literature claws back. Alongside it, two papers land on the same morning with opposite verdicts on whether privileged distillation teaches anything at all, a training-free positional trick recovers memory that a video model had silently lost the ability to look up, and one extra exponent in the Chinchilla scaling law buys a 10x cheaper answer to the highest-leverage question in any training program: which run should we do. The influence axis shows up once, in Google dismantling DeepMind.
...
- **SMRC-SD**: distil an agent only at turns where the reference trajectory actually covers the state it reached. ALFWorld 0.746 to 0.865.
...
**The privileged-distillation cluster spent two weeks refining a filter and today two papers arrived with opposite verdicts on whether there is anything to filter.** [Privileged, but Biased](../../inference-efficiency/2026-08-10-privileged-but-biased-self-distillation.md) reproduces the published gains on easy tasks and finds that on hard ones the loss falls steadily while accuracy does not move, because conditioning a teacher on one reference solution pulls its per-token target toward that trajectory rather than toward correctness, so the loss mass lands on stopwords and punctuation and the exploratory tokens inside *correct* rollouts take the largest penalty. [SMRC-SD](../../ai-routing/2026-08-10-smrc-sd-state-matched-routing.md) reports +12 points on both ALFWorld and WebShop by gating distillation to turns where the reference actually covers the state the agent reached, which is exactly the objection [TurnSight (08-05)](../../inference-efficiency/2026-08-05-turnsight-turn-level-hindsight-distillation.md) raised when it argued that a privileged context built from the ground-truth answer describes the answer rather than the agent's situation. Either state matching incidentally defuses the reference bias, which nobody has claimed, or SMRC-SD's environments sit in the same easy regime where the negative result also reproduces gains, and the clean experiment is to measure the bias score inside [RSTG (08-06)](../../inference-efficiency/2026-08-06-rstg-negative-group-teacher-guidance.md)'s negative-zero-variance support, meaning the prompts where the whole sampled group failed and the reward gradient is dead. The industry side is quietly on the negative result's side already: ByteDance's founder ruled out distillation as a shortcut on 08-06 even at the cost of lagging domestic rivals, and the labs that keep shipping frontier capability keep paying for verifiable rewards rather than replacing them.

**Memory compression is being attacked at three layers this week and the cheapest fix is the one nobody knew was needed.** [WorldTrace](../../inference-efficiency/2026-08-10-worldtrace-addressable-kv-memory.md) finds that a video model's compressed visual cache becomes literally unreachable past the training horizon because the rotary positional offsets it uses as lookup keys drift out of distribution, and that compressing in rotated space corrupts entries by blending incompatible phases, fixed training-free by assigning each summary slot a fake in-distribution position; [StreamArena](../../agentic-systems/2026-08-10-streamarena-streammind.md) independently measures the same symptom from the understanding side, listing "repeatedly compress visual memory and fine detail decays" as one corner of its three-way streaming tradeoff; and the **Zero-Mem** result in this week's DAIR.AI roundup reports memory-operation time cost falling 57.6% by spending **zero LLM tokens** on anything except the final answer, indexing raw traces twice instead of generating summaries. Against that, the industry is buying memory the expensive way: production memory stacks pay per-call for summarization, writing and reranking, which Zero-Mem argues is mostly purchasing structure that plain indexing already provides. The pattern across all three is that a serving-layer or indexing-layer change is beating a modeling change, which is the same lesson [Mixture-of-Kittens (08-05)](../../inference-efficiency/2026-08-05-mixture-of-kittens-moe-megakernel.md) taught on the training side when fusing MoE dispatch into one kernel bought 1.41x end to end with no architectural change at all.
...
- **The distillation cluster's central question gets settled by a bias measurement, not by a new filter.** Within 60 days, watch for any paper that reports a PI Bias Score, or an equivalent reference-dependence metric, computed inside an RL-plus-distillation setup rather than for distillation alone. If that measurement comes back low on negative-zero-variance prompts, the correct form of the whole family is "distil only where the verifiable reward is silent," and the eight filtering axes catalogued in [knowledge-distillation.md](../../inference-efficiency/knowledge-distillation.md) were solving a problem that only exists when distillation runs alone.
- **WorldTrace's addressability failure shows up in a text long-context paper within 90 days.** The signal: any paper reporting that long-context retrieval degrades past the training horizon **for positional-addressing reasons distinct from the usual length-extrapolation loss degradation**, or any serving framework changelog that adds virtual-position assignment to its KV compression or offload path. If neither appears by 2026-11-10, the effect is probably specific to how video world models reuse temporal RoPE and does not generalize.
