...
- **The full cheap-agent-supervision recipe ships as one method within 60 days.** All four components now exist and several come from adjacent author groups: pivotal turns from AgentOPSD (08-07), pivotal tokens from TIP and OPD², the teacher-minus-base delta from OPD², and now the state gate from SMRC-SD. The signal: an agentic-RL paper citing turn-level credit and token-level selection together and reporting **reduced rollouts and reduced training tokens simultaneously** on ALFWorld or WebShop. If that paper also reports a reference-bias measurement, the two halves of today's contradiction get settled in one shot.
...
- **Somebody names compressed-state addressability as one modality-general principle within 90 days.** Raven found it in a language model's recurrent state on 08-04 and WorldTrace found it in a video model's KV cache today, independently and without citing each other, which is usually one paper away from being given a name. The signal: a position piece or survey framing addressability as the governing constraint on state compression, or a hybrid model shipping addressable slot-state with differential checkpointing. If instead the next three compression papers keep reporting ratios computed in rotated space, the field has not noticed yet.
