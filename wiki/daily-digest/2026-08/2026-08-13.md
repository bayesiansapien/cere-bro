...
<ol>
<li><span class="verb read">Read</span><strong>AI4AI at Test-Time.</strong> A strong model writes an inference-time harness for a weak one and accuracy jumps 0.49 → 0.91 with the weights frozen. Inference efficiency meeting your harness and distillation lines, and the capability twin of the 5–30x cost number. <a href="https://arxiv.org/abs/2608.12307">Paper</a> · <a href="../../inference-efficiency/2026-08-13-ai4ai-test-time-harness-transfer.md">wiki</a>.</li>
...
- **One model runs across five harnesses on Terminal-Bench 3.0 within 60 days, and the harness effect gets isolated for the first time.** The current leaderboard reports every row as a model-harness pair with tokens and dollars, but **no model appears under two harnesses in the top ten**, so it shows harness variation exists without measuring it. The signal: any published run holding a single model fixed across mini-SWE-agent, Codex, Claude Code, Grok Build and Cursor CLI with resolution rate and cost for each. If the spread is under 2x, omarsar0's 5x-to-30x cost-per-success swing is task-specific rather than general and the harness discipline is being oversold. If it exceeds 2x, every model-versus-model leaderboard published this year is confounded.
