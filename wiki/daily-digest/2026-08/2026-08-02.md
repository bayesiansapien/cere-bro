...
- **Agent authority splits in two directions**: an immutable effect ceiling that survives the agent evolving, and per-field certification of tool-call arguments.
- **MAPD**: distil a closed proprietary model through a JSON protocol. No logits needed, and it transfers across different teachers.
...
**Still on the boards and still unread.** ***Reality Monitoring in Large Language Models*** (2607.23927, cs.AI #8) asks whether a model can tell its own prior output from what the user said, finds that source attribution collapses once episodic delay removes the easy shortcut, and reports that feedback makes some models swap internal and external judgments while others improve accuracy as confidence decouples from correctness. That is a failure mode invisible to every benchmark on the [agent-memory page](../../agentic-systems/agent-memory.md) and it should be read next. ***Efficiency Matters in Autonomous Research*** (2607.24647, cs.AI #3, 84.0% win rate, the highest on either board) argues that autonomous research systems are judged only on final outcome quality and should be judged on the area under the Pareto frontier of their search, and introduces a portfolio-bandit "fluid search" that matches a per-task oracle. Given that the [shadow-evaluation result (07-30)](../../agentic-systems/2026-07-30-shadow-evaluations-ai-research-agents.md) found frontier agents doing all of the engineering unassisted and having both outputs rejected by the original authors, a paper arguing that search efficiency and outcome quality are distinct dimensions belongs in the same reading.
