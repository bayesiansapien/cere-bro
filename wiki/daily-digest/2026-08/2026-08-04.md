...
- **ScrambleToolBench**: agents discover hidden tool behaviour, then cannot adapt when it changes. More reasoning just widens the brute-force search.
...
- **Celeris-1 claims the fastest model spot on Artificial Analysis** at 2,038 output tokens per second, 2x the next fastest, beating Grok 4.3 on HLE, explicitly without custom silicon ([@Celeris_ai](https://x.com/Celeris_ai/status/2084429307422872026)).
...
- **Rising authors from Kurate.** The threshold crossings this week are unchanged from 08-03 and remain the biomedical foundation-model cluster that has now held the top of both boards for three straight weeks: **Guy Lutsker, Gal Sapir, Jordi Merino, Smadar Shilo, Anastasia Godneva** and **Eli Meirom** on *Simulating clinical interventions with a generative multimodal model of human physiology* (ai_rating 7.8, cs.AI #1 for weeks 29 through 31). No new names crossed. The falsifiable filter stays the same as last week: **if that group posts a follow-up by 2026-10-01 reporting inference cost or serving latency for a patient-scale temporal model, it becomes relevant here**, because a foundation model over a health system's full longitudinal record is a long-context KV problem wearing a clinical label. Absent that, no additions to `connectors/twitter/config.json:ai_handles` are warranted. Worth noting separately that today's most valuable paper, Raven, came from the cs.LG board at rank 16 rather than rank 1, and its lead author group (Afzal, Bick, Gu) has now produced two entries the wiki rated highly while never appearing on HuggingFace. **Albert Gu and Aviv Bick are better handle candidates than anyone the rising-author threshold has surfaced in a month.**
