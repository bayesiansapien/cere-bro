...
> Two independent groups derived the same routing formalism this week without citing each other. This one runs it inside a single model, and it has a training signal the cross-model version can never get.

**Source:** Kurate cs.LG (LLM-rated, never appeared on HuggingFace). Surfaced because co-author Daniel Whitmore crossed the Kurate rising-author threshold this week.
**Links:** [Paper](https://arxiv.org/abs/2608.02528) · [Wiki summary](../../ai-routing/2026-08-25-voi-routing-mixture-of-lora-experts.md)
...
**What is it about?** Mixture-of-LoRA-Experts systems freeze one large base model and stack a pool of small low-rank adapters (LoRA, a cheap way to specialize a model by training a small weight patch while the backbone stays frozen) on top, with a router picking a subset per input. The live question is how many adapters each input should get.
...
**Key takeaways**
- Same formalism as Pandora's Router above, derived independently, one level down the stack. Neither paper cites the other.
...
→ [Full summary](../../ai-routing/2026-08-25-voi-routing-mixture-of-lora-experts.md)
