...
- **Encrypted reasoning traces are stealable.** Feed one to a weaker model from the same provider and it reads the plaintext aloud. Works on Anthropic, OpenAI and Google.
...
**Source:** HuggingFace + Kurate cs.LG #8
...
**Source:** Kurate cs.LG #15, absent from HuggingFace
...
- **The full tech report carries the ablations the abstract hides, and two of them are the interesting part.** Hugging Face's Elie Bakouch pulled Figure 6, from controlled experiments at roughly 10B parameters. Expert-Specific PolyNorm holds a **higher mean effective rank in the expert gate weights than SwiGLU at every layer** (about 180 versus 163 at layer 2, about 115 versus 70 by layer 15), meaning a flatter singular-value spectrum and less collapsed expert specialization. And Motif 3 uses **decaying router noise**: Gaussian noise added to the FP32 router logits on a cosine decay schedule, large early to stop routing locking in before experts have specialized, annealed away once roles form. Maximum tokens per expert falls to the median-load regime by about 17B training tokens with noise versus about 25B without. Bakouch notes he has not seen router noise defined in a big-model report before, possibly since early Google MoE work.

**Gaps in the study**
No ablation isolates GDLA's two halves against each other, so what fusing differential attention with latent KV compression buys over either alone is still unmeasured, even though the report does ablate GDLA as a unit. No cache-per-token or KV-throughput figure is published, which is the number that would let anyone compare it to the sparse-attention family, and it leaves this wiki's standing gap open in exactly the model that could have closed it. And MOPD's contribution is not separated from the cost of simply having run seven RL jobs, so whether multi-teacher distillation consolidates capabilities or averages them is still open.
...
### Macaron-V1: One LoRA Per Turn on a Frozen 744B Base

> Every router this wiki tracks pays a prefix-cache penalty when it switches. This one swaps a small weight patch on a base model that never moves, which is the cheapest route-switch anyone has proposed.

**Source:** HuggingFace
**Links:** [Paper](https://arxiv.org/abs/2608.09819) · [Wiki summary](../../ai-routing/2026-08-11-macaron-v1-mixture-of-lora.md)

```mermaid
flowchart LR
  T[User turn] --> SEL{Per-turn<br/>LoRA selection}
  BASE[(Frozen base:<br/>744B GLM-5.2)] --> SEL
  ...
  L3 --> OUT
  L4 --> OUT
  OUT --> EXP[Experience under<br/>versioned contract]
  EXP --> EVAL{Scored by something<br/>the system<br/>does not control}
  EVAL --> NEXT[Successor<br/>model-harness pair]
  classDef input fill:#dbeafe,stroke:#3b82f6,color:#1e3a8a
  classDef decision fill:#fef3c7,stroke:#f59e0b,color:#78350f
  classDef output fill:#d1fae5,stroke:#10b981,color:#065f46
  classDef aux fill:#e0e7ff,stroke:#6366f1,color:#312e81
...
**What is it about?**
An open agent-model family whose headline architecture is a routing decision. A very large base model is frozen, a handful of specialist LoRA adapters (small low-rank weight patches that adapt a frozen model without retraining it) sit on top, and exactly one adapter is selected per user turn. The flagship pairs a 744B GLM-5.2 base with four adapters for chat, agent, coding and generated UI; a 50B Qwen3.6 version runs the same design locally.

**What problem does it solve?**
One model has to be good at several very different jobs, and training one set of weights for all of them averages them together. The usual alternative is to route across separate models, which means separate weights resident somewhere and a prefix-cache invalidation every time the route changes mid-conversation. Mixture-of-LoRA keeps the base weights and the base cache in place and swaps only the patch.

**What is the core novelty?**
The routing granularity. This wiki has logged five routing axes: model-tier, task or expert, attention-head, trajectory, and latent-trajectory. Adapter-level routing over a shared frozen base is a sixth, and it is the only one whose switch cost is a small weight load rather than a re-prefill. The second half of the system is a recursive loop that versions the model-and-harness together and scores each successor against an external contract.

**Key takeaways**
- The routed unit is a **set of weights on a shared base**, not a whole model, and the routing decision fires once per conversational turn.
- It converges with [A²E (08-11)](../../agentic-systems/2026-08-11-harness-evolution-cluster.md), the harness auditing engine that found no model-harness combination wins across all task types, on the same conclusion from a different direction: the unit worth versioning is the model-harness pair, not the model.
- Role-based model assignment now has a sixth independent datapoint after [Kilo's plan-strong/implement-cheap split (06-16)](../../ai-routing/2026-06-16-kilo-plan-implement-model-split.md) at 59% cheaper with identical acceptance results, [Sakana's Conductor (05-11)](../../ai-routing/2026-05-11-conductor-sakana-orchestrating-frontier-models.md) where a 7B orchestrator beat every frontier worker it directed, and [DSPy at Shopify (07-25)](../../ai-routing/2026-07-25-dspy-task-model-separation-550x.md) at a 550x cost gap from fixing the task contract then searching for the cheapest passing model. This is the first that ships the assignment inside one served model instead of across an API boundary.
- The authors state plainly that compounding gains from continual learning remain an open question, which is unusual candour for a flagship model report.

**Gaps in the study**
The selection mechanism is the least-specified part of a paper whose headline is selection: nothing is said about whether the per-turn choice is learned or prompted, what it costs, or how often it is wrong. No adapter-switch latency or cache-behaviour number is published, which is precisely the figure that would prove the cheap-switch claim rather than imply it. Four adapters is too few to constitute a real routing problem, and no ablation separates the adapter scheme from the very large base it sits on.

**Industrial implication**
If the cache-preserving claim holds, this is the routing architecture that gets deployed, because it is the only one that does not pay for switching. The thing to watch is adapter count: at four this is specialist selection, and somewhere past a few dozen it becomes a genuine router with a real decision problem and the paraphrase-robustness worries that [When Is Routing Meaningful? (07-20)](../../ai-routing/2026-07-20-when-is-routing-meaningful.md) raised about learned routers collapsing under rewording.

→ [Full summary](../../ai-routing/2026-08-11-macaron-v1-mixture-of-lora.md)
...
> Providers hide chain-of-thought by encrypting it and handing the ciphertext back to the client. The blocks turn out to be interchangeable across sessions, users and models, so you can feed a strong model's encrypted trace to a weaker sibling and ask it to read the thing aloud.

**Source:** HuggingFace
**Links:** [Paper](https://arxiv.org/abs/2608.09867) · [Wiki summary](../../responsible-ai/2026-08-11-stealing-reasoning-traces.md)

```mermaid
flowchart LR
  S[Strong model<br/>produces reasoning] --> ENC[Encrypted block<br/>returned to client]
  ENC --> CL[Client passes it back<br/>with each request]
  ENC --> INJ[Inject into a weaker,<br/>less safeguarded model<br/>same provider]
  INJ --> PLAIN[Model decodes and emits<br/>the trace verbatim]
  PLAIN --> V1[Anti-distillation<br/>bypassed]
  PLAIN --> V2[367 PII artifacts<br/>182 credentials<br/>from 315,320 public blocks]
  PLAIN --> V3[Hazardous content the<br/>visible answer refused]
  PLAIN --> V4[Invisible prompt injection<br/>hidden inside the block]
  classDef input fill:#dbeafe,stroke:#3b82f6,color:#1e3a8a
  classDef warn fill:#fee2e2,stroke:#ef4444,color:#7f1d1d
  classDef aux fill:#e0e7ff,stroke:#6366f1,color:#312e81
  class S,ENC,CL input
  class INJ,PLAIN aux
  class V1,V2,V3,V4 warn
```

**What is it about?**
Frontier providers stopped showing users the model's step-by-step reasoning, to protect intellectual property and limit leakage. Rather than keeping those traces on their own servers, they encrypt them and return the ciphertext to the client, which passes it back on each subsequent request. This paper attacks that design.

**What problem does it solve?**
It does not solve a problem, it finds one. The architectural flaw is that the encrypted blocks are **fully interchangeable across sessions, users and models inside one provider's ecosystem**. Nothing binds a block to the conversation, the account or the model that produced it.

**What is the core novelty?**
A decryption jailbreak that never touches the model being stolen from. Inject a strong model's encrypted reasoning trace into a **weaker, less safeguarded model from the same provider**, and that model decodes and emits the trace verbatim in plaintext. The capable model is never jailbroken, so its safeguards are irrelevant.

**Key takeaways**
- Demonstrated across **Anthropic, OpenAI and Google**, which makes this an industry-wide design pattern failing rather than one vendor's bug.
- Decoding **315,320 reasoning blocks scraped from public repositories** recovered **367 personally identifiable information artifacts and 182 credentials**, because developers share session logs without knowing what the opaque blocks contain.
- It surfaces hazardous content that appeared inside the reasoning **even where the visible final answer correctly refused the request**.
- It enables **invisible prompt injection**: a payload embedded entirely inside an encrypted block, which poisons public agentic rollouts without being readable by anyone inspecting the log.
- Responsibly disclosed, with cryptographic and system-level mitigations proposed.

**Gaps in the study**
The paper does not report whether the vulnerability survives the mitigations it proposes, and there is no measurement of how much of a proprietary model's reasoning capability an adversary could actually reconstruct from extracted traces, which is the number that decides whether this is an IP problem or only a privacy problem.

**Industrial implication**
This lands on three of today's other stories at once. It **circumvents anti-distillation mechanisms directly**, which is the exact protection Anthropic asked regulators to reinforce, and it arrives the same day Anthropic announced [invisible watermarks in all Claude output](https://support.claude.com/en/articles/16266773-how-claude-marks-ai-generated-content) as a provenance measure. It is also a credential leak at scale hiding inside artifacts everyone treats as opaque, and it makes the agent-security incidents of the past 24 hours look structural rather than incidental. The immediate operational advice is blunt: **treat encrypted reasoning blocks in any public repository as unredacted logs.**

→ [Full summary](../../responsible-ai/2026-08-11-stealing-reasoning-traces.md)
...
- **tinygrad demoed Qwen 3.6 27B on an AMD 7900XTX over USB3 at 34 tokens/s**, with the eGPU dock launching on the 12th with fully open-source firmware ([@\_\_tinygrad\_\_](https://x.com/__tinygrad__/status/2086896454376006026)). The posted terminal shows IQ4_XS weights at 131K max context, prefill between 30 and 346 tokens/s depending on input length, and generation pinned at 33 to 34 tokens/s.
- **tinygrad now lowers kernel dispatch itself into its compiled graph**, treating GPU command-queue submission as just another program, the same launch-overhead target TileRT attacks commercially ([@\_\_tinygrad\_\_](https://x.com/__tinygrad__/status/2086863031766827406)).
- **Apple approved tinygrad's eGPU driver for both AMD and NVIDIA cards on Mac**, opening external-GPU local inference to Thunderbolt and USB4 machines ([@\_\_tinygrad\_\_](https://x.com/__tinygrad__/status/2039213719155310736)).
- **Bernie Sanders sent letters to the CEOs of OpenAI, Anthropic and other frontier labs**, adding Senate pressure to the week's safety-departure story ([@ns123abc](https://x.com/ns123abc/status/2086858766041993357)).
- **Muse Glimmer is architecturally close to Gemma 4**, per Hugging Face's Elie Bakouch: distilled from Muse Spark, 52 layers against Gemma 4's 60, nearly twice the vision layers at 50, a 2x larger sliding-window at 2K, and half the attention width at 4K ([@eliebakouch](https://x.com/eliebakouch/status/2086769240271405477)).
- **South Korea's ruling party moved to ban AI sexual content even when no real person is depicted**, after a court acquitted a deepfake-sharing defendant on the grounds the women were virtual ([@MarioNawfal](https://x.com/MarioNawfal/status/2087074960250921092)).
- **X's Q2 ad revenue came in at $367 million**, against $1.08 billion at Twitter in Q2 2022, per SpaceX earnings ([Techmeme](https://x.com/Techmeme/status/2086982391315513448)).
...
- **At least one major provider binds encrypted reasoning blocks to a session or model identity within 60 days, or the anti-distillation defense is conceded as unfixable-in-practice.** [Stealing Reasoning Traces](../../responsible-ai/2026-08-11-stealing-reasoning-traces.md) showed the blocks are interchangeable across sessions, users and models at Anthropic, OpenAI and Google, so the fix is known and the only question is whether anyone pays for it, since session-bound blocks break the multi-session agentic workflows that client-side reasoning state exists to support. Two signals, either counts: an API changelog noting that reasoning blocks are no longer portable across models or sessions, or a documented error when replaying a block into a different model in the same family. If instead the labs respond only with detection, meaning more watermarking of the kind Anthropic shipped on 08-10, that is a concession that prevention lost and the provenance fight moves entirely to forensics.
...
*Coverage note: all eight `raw/reddit/2026-08-11-r-*.md` files returned zero posts passing filters (LocalLLaMA, MachineLearning, MLScaling, CUDA, LLMDevs, ControlProblem, HPC, reinforcementlearning), the third consecutive empty day, so there is no practitioner ground truth in today's digest. `raw/twitter/2026-08-11-morning.md` captured 97 tweets with zero `@bayesiansapien` retweets, so the curated-signal layer is absent and Twitter contributes only AI-handle items; the afternoon slot added five tweets with no AI research signal. RSS published only three items dated 2026-08-11 by early afternoon, so the RSS layer here is mostly 2026-08-10 files. The HuggingFace board grew from 17 to 20 papers during the day; Stealing Reasoning Traces, Macaron-V1, ConCor-1 and CEAA were added in a second pass. Of the late arrivals, ConCor-1 (bidirectional vision-language concept correspondence, +48% correspondence F1 on long captions) and CEAA (a modular cognitive architecture for embodied virtual agents) sit outside this wiki's attention range and are recorded here rather than given Deep Dives.*
