...
### Memory Technology for Agentic AI: the compute story was always a memory story

> The 2026 infrastructure question is no longer how many FLOPS you have. It is where the weights, the KV cache, the retrieval data, and the tool state live, and how fast they move. Agentic AI touches every memory tier at once, and the one that now dominates is the KV cache.

**Source:** Ken Huang / DistributedApps.ai (RSS)
**Links:** [Post](https://kenhuangus.substack.com/p/memory-technology-for-agentic-ai) · [Wiki summary](../../hardware/2026-06-07-agentic-ai-memory-hierarchy.md) · [Concept: memory hierarchy](../../hardware/memory-hierarchy.md)
...
flowchart LR
  ...
  Q -->|orchestration, RAG| DDR[DDR5 / MRDIMM<br/>+ LPDDR5X SOCAMM2]
  Q -->|capacity, pooling| CXL[CXL memory]
  Q -->|KV overflow, context| SSD[NVMe SSD +<br/>NVIDIA CMX tier]
  HBM --> KV[KV cache is now the<br/>dominant memory traffic]
  SRAM --> KV
  SSD --> KV
  WALL[HBM-only<br/>mental model] -.->|stranded, fragmented| FAIL[Idle accelerators,<br/>broken token economics]
...
**What is it about?**
A technical-and-business survey of the memory tiers AI runs on, from on-chip SRAM through HBM (high-bandwidth memory stacked next to the GPU), GDDR, DDR and low-power LPDDR, CXL (coherent PCIe-attached memory), and SSD context storage. The argument is that agentic workloads, which reason over long horizons, hold cross-turn memory, call tools, and run several inference passes per answer, stress all of these at once rather than just the accelerator.

**What problem does it solve?**
It reframes the "we just need more GPUs" story. NVIDIA says agentic systems consume up to 15 times more tokens than ordinary AI apps, and a 2026 UC Berkeley report restates the systems reality: as context grows, generation becomes memory-bandwidth-bound and the dominant traffic shifts from model weights to the KV cache (the stored attention keys and values that let a model avoid recomputing past tokens). Memory placement and bandwidth, not raw compute, decide token economics.

**What is the core novelty?**
Not a new device but a clear synthesis that puts numbers on why no single tier wins. Micron's SOCAMM2 (a low-power LPDDR server module) claims more than 2.3x better time-to-first-token when used for KV-cache offload at one-third the power of standard RDIMM. Groq's LPU keeps hundreds of MB of state in on-chip SRAM at 150 TB/s to kill latency and jitter. NVIDIA's CMX turns SSDs into an AI-native ephemeral-KV tier. Each is a different answer to "the KV cache will not fit in HBM."

**Key takeaways**
- KV cache, not weights, is the bottleneck tier at long context. This is the hardware fact under every KV-cache software paper the wiki tracks (eviction, quantization, low-rank latent caches, context compression).
- The shortage is structural, not a normal cycle. Micron cites a roughly 3-to-1 HBM-to-DDR5 wafer trade ratio that worsens each HBM generation, so building HBM directly shrinks how much conventional DRAM the industry can ship.
- Relief comes in waves: 2026 stays tight and allocation-driven, 2027 brings selective relief, 2028 to 2029 is the earliest broad relief, and top-end HBM stays allocation-driven into 2030.

**Gaps in the study**
It is a survey, so the forward timing (2028 to 2030) is a vendor-sourced projection, not a measured result, and it leans on supplier claims (Micron, NVIDIA, Samsung, Groq) that each have a product to sell. There is no independent benchmark comparing the tiers on a fixed agentic workload.

**Industrial implication**
If HBM is allocation-driven for five years, serving stacks cannot assume abundance. They must tier and ration memory the way the demand-side research already rations compute. The open systems lever is KV-aware tiering: deciding per request which KV blocks stay in HBM versus spill to LPDDR, CXL, or SSD. That is the hardware dual of [CLEAR](../../inference-efficiency/2026-06-05-clear-shadow-price-reasoning-budget.md)'s per-query compute rationing (06-05, ration a fixed inference-token budget by marginal utility), and it is where the next round of serving-stack engineering will concentrate.

→ [Full summary](../../hardware/2026-06-07-agentic-ai-memory-hierarchy.md)
...
**Source:** HuggingFace Daily Papers
**Links:** [Paper](https://arxiv.org/abs/2606.06286) · [Wiki summary](../../responsible-ai/2026-06-05-propme-memorization-propensity.md)
...
→ [Full summary](../../responsible-ai/2026-06-05-propme-memorization-propensity.md)
...
**Research spent the week deriving how to do more with less compute, exactly as the market repriced the bet that more compute is the whole game.** Today's efficiency cluster is unusually coherent, and the Ken Huang memory survey names the hardware fact under all of it: at long context the dominant memory traffic shifts from weights to KV cache, and HBM stays allocation-driven toward 2030. On top of that floor sit SEAOTTER (compress sensor data 200:1 and exit to JPEG), Flash-WAM (distill a robot policy to one step for a 23x speedup), and the Kilo Code audit (a 7-cent open model catching the same bugs as a 70-cent frontier run). Against them, Gary Marcus's "AI's Black Friday" (a ~$500B drop, Musk renting out the GPUs he hoarded, OpenAI courting a government equity stake) is the market saying the brute-scale capex thesis is cracking, and Sakana's new recursive-self-improvement lab is a frontier player betting explicitly that self-improvement, not raw compute, is the next lever. The same scarcity shows up on the demand side in Copilot's switch to usage-based billing and Uber's exhausted budget. Research is supplying the per-watt, per-byte efficiency toolkit at precisely the moment the economics demand it.
...
- **KV-aware memory tiering ships in a major serving stack within 90 days.** If HBM is allocation-driven toward 2030 and KV cache is the dominant traffic (per the Ken Huang survey), the serving layer must decide per request which KV blocks stay in HBM versus spill to LPDDR, CXL, or SSD. **Signal:** vLLM, SGLang, or TensorRT-LLM adds a first-class KV-offload-to-LPDDR/SSD policy or a SOCAMM2/CMX-style tiering hook, not just flat eviction.
