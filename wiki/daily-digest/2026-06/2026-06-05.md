...
The distillation story took its sharpest turn of the spring today, and it turned by moving house. All season the on-policy distillation thread (training a small student on its own outputs under a teacher's token-level supervision) argued about *which output tokens* carry the signal: TIP (04-16) said under 10%, TA-OPD (06-01) said only the reachable ones, TrOPD (06-03) said only the reliable ones, FiRe-OPD (06-04) said soft-weight the survivors. OPRD ends that argument by leaving the room. It stops matching output probabilities and instead aligns the student's and teacher's *hidden states* on the same rollouts, which kills the Monte-Carlo variance of estimating a reverse-KL over a 150,000-token vocabulary and finally closes the student-teacher gap on AIME where output-space methods plateau below the teacher. And on the same day, a second self-evolving-agents paper (Continual Experience Internalization) reports the opposite of the spring orthodoxy: for *iterative* experience transfer, off-policy context distillation is more stable than on-policy, which collapses over rounds. The field spent three months refining on-policy selection; today one paper changed the space and another questioned the policy.

That tension sits directly under the day's loudest industry signal. Anthropic published "When AI builds itself," claiming Claude now writes over 90% of Anthropic's code, engineers ship 8x more code per quarter than in 2021-2025, and a preview model improved on human researchers' next-step decisions 64% of the time, up from 22% in 2024. That is the industrial face of models-training-models, and the distillation thread is its academic substrate. The gap between them is clean: Anthropic measures the *outcome* (acceleration toward recursive self-improvement) while the research is still arguing about the *mechanism* (which space, on-policy or off). Underneath, a quieter convergence: four papers today (MLEvolve, EvoDS, Continual Experience Internalization, MMPO) all attack sustained agent self-improvement, the same week NVIDIA shipped Nemotron 3 Ultra "for long-running agents" and Prime Intellect handed 2,500+ reinforcement-learning environments to the Nemotron Coalition.

The third thread is economic. CLEAR reframes inference-time compute as a budget-allocation problem across a batch of queries, equalizing the marginal value of a token via a single "shadow price" and abandoning queries that cannot be solved within budget, for up to 3x higher accuracy under scarcity. It lands the same week GitHub Copilot moved to usage-based billing and the SpaceX-Cursor compute deal signaled that the era of free compute is over. Research is deriving the optimal way to ration thinking exactly as industry starts charging for it. Kurate's weekly leaderboard stayed almost entirely clinical with zero HuggingFace overlap, so there is no cross-source-confirmed paper today, and all eight Reddit subs were empty this window, leaving today's claims without practitioner ground-truth checks.
