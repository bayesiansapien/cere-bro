...
```mermaid
flowchart TB
  C[Cached KV<br/>token budget B] --> G{Value magnitude<br/>guard}
  G -->|large value states| P[PROTECTED<br/>never evicted]
  G -->|rest of cache| S[Stochastic eviction<br/>rank, then sample what to drop]
  P --> F[Static cache<br/>FlashAttention2-compatible]
  S --> F
  X[Evicting a large-magnitude<br/>value state] -.->|triggers| Y[Repetitive-loop collapse]
```
...
```mermaid
flowchart LR
  R[Student rollout<br/>token t] --> Check{Inside teacher<br/>reliable region?}
  Check -->|yes| A[Apply OPD loss]
  Check -->|no| C[Clip ratio, mask,<br/>or forward-KL]
  T[Off-policy guidance:<br/>continue from teacher prefix,<br/>imitate via forward-KL] -.->|pulls exploration back| A
```
...
```mermaid
flowchart TB
  D[30T human tokens<br/>AI text scrubbed<br/>no third-party distillation] --> B[MAI-Base-1]
  B --> S1[STEM and<br/>competitive code]
  B --> S2[Agentic coding<br/>and tools]
  B --> S3[Helpfulness<br/>and safety]
  S1 --> M[Supervised<br/>consolidation]
  S2 --> M
  S3 --> M
  M --> F[Final RL climb]
  F -->|GRPO with asymmetric trust region<br/>+ hard ratio clamp<br/>+ entropy integral controller| T[MAI-Thinking-1<br/>1T params, 35B active]
```
