...
- **A third modality gets a hypernetwork-predicted adapter within 60 days, confirming parametric context internalization as a general interface.** Code and video both landed today after the Doc-to-LoRA text precedent; the move (predict an adapter from context, serve with zero tokens) is modality-agnostic in principle. **Signal:** a paper applies the same predict-a-LoRA-from-context recipe to audio, tool-call histories, or tabular data, OR a serving framework adds "internalize this document/repo into an adapter" as a first-class API alongside RAG.
