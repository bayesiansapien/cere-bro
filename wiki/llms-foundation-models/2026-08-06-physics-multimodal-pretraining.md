...
**The 5%-compute recipe belongs on the efficiency beat, not just the multimodal one.** Most compute-efficiency results in this wiki are inference-side. This is a pretraining-budget result with a scaling validation attached, which makes it comparable to [OmniPack (08-05)](../inference-efficiency/2026-08-05-omnipack-omni-modal-token-compression.md), the training-free token compression for omni-modal models that keeps 98% of performance at 16.7% of the FLOPs. Different stage, same shape of claim: a large fraction of the compute currently spent on multimodal models is buying very little.
