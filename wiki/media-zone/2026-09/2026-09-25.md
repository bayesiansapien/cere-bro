...
- **The US day's conversation, now about 2pm ET:** acceptance announcements dominate raw volume, and almost none of them carry a number. The freshest wave is the decision-model race itself: a sub-6B challenger just took first place on the Decision Index by using a fifth of the tokens per decision.
...
- **Harness beats model, again:** Alibaba's open code reviewer matched a frontier agent on precision with about 9x fewer tokens, the fifth harness-first result of the day.
...
- **The leaderboard already turned over.** Nace.AI's Drex, under 6B parameters, edged the hosted model on the Decision Index, 51.73 to 51.67, winning 23 of 40 scored benchmarks. The margin is noise. The real claim is 70 tokens per decision against 367, about 25 decisions a second on one H100. That is a token-efficiency win, not an accuracy win, and it arrived via a promotional head-to-head game, so wait for the index maintainers to confirm.
- **Valen puts eyes on a decision model.** A vision encoder, a Qwen3.5-2B backbone and a shared decision head, trained with SFT, then GRPO, then RLCD (reinforcement learning for calibrated decisions), all open. It clears a 10-step Sokoban puzzle in 1.13 seconds where a 27B model in thinking mode needs 198 seconds. It is the first open multimodal entry, answering this morning's complaint that the category takes no images.
- **LlamaIndex published the operator's version of the cascade.** Pick a precision target, say 97 percent of auto-accepted extractions correct, sweep the confidence cutoff upward until you hit it, validate on a held-out set, then re-run as documents drift. It is the CMU cascade written as a runbook, and it only works if the scores are calibrated, which is AnyJev's whole point.
...
- **The quantization push reached agents.** OrcaSAQ-2, a 3.21-bit Qwen3.8-27B, claims 58.4 on Terminal-Bench 2.1 (against 58.5 for Sonnet 4.6 in Claude Code), 70 on SWE-bench Verified and 262K context in 12 GB. It came from a promotional account with no link, so every number is unverified. If it holds, it is the agentic counterpart to Mirai's 2.4-bit codec from 09-24, which pushed a frontier-class model below three bits per weight. Cost angle: a coding agent on one consumer GPU.
...
- **Alibaba open-sourced the code reviewer its engineers have used for two years,** and it is the cleanest harness-beats-model result yet. Plain code picks which files and rules apply, related files go to sub-agents with clean contexts, and the LLM only makes judgment calls. On 200 real pull requests it beat Claude Code on precision with the same underlying LLM and about 9x fewer tokens.
- **Anthropic's Opus 5.5 prompting guide says to delete "think step by step."** The model decides how much to think, so the lever is the effort setting, and default medium matches or beats Opus 5 on high in Anthropic's tests. The rest is harness advice: a checklist for long tasks, explore context before acting, and a time budget for multi-agent runs ([guide](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5-5)).
- **LangChain named the stack of loops.** Agent loop, then a verification loop with a grader, then an event-driven loop that runs on crons and webhooks, then hill-climbing that improves the harness itself from traces. The claim is that value compounds in the outer loops, which is exactly what the medical-receptionist system above is doing.
- **SREGym got into NeurIPS** as a benchmark for what happens after deployment: live environments with metastable failures and misconfigurations, asking whether agents can resolve production incidents rather than write code.
- **Influence angle:** five independent groups reached "the harness is the product" in one week. That is past the threshold where it is a trend and into the range where it is the default frame for the next quarter of tooling.

<div class="mz-links">

[@iiiichigo_chan (Boris Cherny talk)](https://x.com/iiiichigo_chan/status/2103142402047287534) · [@callanxai (Andrew Ng)](https://x.com/callanxai/status/2103193192325603645) · [Raschka video](https://youtube.com/watch?v=tfBT22RWYww) · [@polydao (Opus 5.5 playbook)](https://x.com/polydao/status/2103418985182081141) · [@notsurajgaud (Lilian Weng's harness post)](https://x.com/notsurajgaud/status/2103024758711533851) · [@supremaxbt](https://x.com/supremaxbt/status/2103175139462623254) · [@undefinedKi (Open Code Review)](https://x.com/undefinedKi/status/2103526276006809627) · [@bibryam (loop engineering)](https://x.com/bibryam/status/2103422761016201574) · [@HacksonClark (SREGym)](https://x.com/HacksonClark/status/2103532222737178702)
