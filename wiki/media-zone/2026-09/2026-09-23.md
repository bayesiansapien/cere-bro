...
### Memory crossed half the semiconductor market, and SK hynix named the KV cache

- **Global semiconductor revenue hit $425 billion in Q2 2026 with memory exceeding 50 percent of the market for the first time.** Every serving result above is an argument that the scarce resource is bytes moved rather than operations executed. This is the market pricing that argument in dollars.
- **SK hynix announced an AI memory architecture spanning high-bandwidth flash, processing-in-memory and explicitly "tiered KV cache management."** A memory vendor has put the KV cache on a product roadmap, which means the GPU-to-RAM-to-NVMe hierarchy KVMEM improvises in software is becoming something you buy.
- **NVIDIA's Vera Rubin NVL72 hit up to 3.7x GB300 throughput in an MLPerf preview**, on a roadmap whose HBM per chip was cut roughly fivefold. That only coheres if the workload is being reorganised around smaller resident caches, which is the same reorganisation every paper here is describing.
- **OpenAI took its Jalapeño accelerator from RTL to tape-out in nine months with LLMs completing the RTL**, and no defect-rate or verification-coverage number was attached. Record the capability, discount the framing.
- **Micron demonstrated a 512 GB DDR5 RDIMM at 9,200 MT/s** and CXMT moved an 11.95 nm G5 DRAM platform into mass production. The second tier that makes offloading viable is getting denser and faster at the same time.

<div class="mz-links">

[Semiconductor Newsletter Week 38](https://thesemiconductornewsletter.substack.com/p/week-38-2026) · [SEMICON India 2026 panel](https://youtube.com/watch?v=CEJJbq-GTN0) · [wiki summary](../../hardware/2026-09-23-semiconductor-week-38-hbf-pim-tiered-kv.md)
...
[![A Perfect World Model of Chess Still Won't Beat Magnus Carlsen](https://i.ytimg.com/vi/qAgzA05Q6z0/hqdefault.jpg)](https://youtube.com/watch?v=qAgzA05Q6z0)
...
- **Machine Learning Street Talk ran two Alexander Mattick episodes** arguing that more data cannot substitute for prior structure, and that a perfect world model of a domain still does not buy good play in it. Both are the theoretical version of Taste-Bench's finding that a larger reasoning budget does not improve mid-trajectory decisions at all, and the second is the sharper of the two. Small audiences so far, which is usually the signal to watch rather than the signal to skip.
