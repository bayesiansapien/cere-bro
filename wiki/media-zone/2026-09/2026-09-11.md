...
- **Meta's Auto-RecSys is the production counterpart**, circulated by @omarsar0 with the line that harness engineering is a top skill right now. It runs autonomous recommender-system experimentation inside Meta rather than as a benchmark, which is the thing most harness papers cannot show. Worth pairing with SoL-Pi: one searched a harness for token cost in a research loop, the other runs one against a revenue surface.
- **The blunt version of the moat argument**, from @gregisenberg: this is the AWS moment for agents, because everything that made agents hard to build (keeping one alive for days, remembering across sessions, recovering from failure) is now something you rent rather than something you build. The framing is promotional but the structural claim matches @aigclink's more careful reading above, and the two arriving independently is the signal.
...
- **Someone open-sourced a whole fruit fly**, not a model of one: a body rebuilt joint by joint from microscopy that walks, grips, flies and lands inside a physics engine. Off your lanes entirely, filed because the simulation-fidelity trend keeps showing up adjacent to embodied-agent work.
- **@ashwingop made the enterprise-agent argument twice**, that bring-your-own-agent will break IT the way bring-your-own-device only annoyed it, and that strong individual agents still cannot do the work *the way a specific company does it* without a shared memory layer. The second post is the more useful one.
