...
### Routing the grunt work: Spotify's 90% token cut, with the numbers

- **The cost framing everyone should steal.** Most of what a coding agent does is not thinking, it is I/O: reading five files to answer a question about one method, generating the twenty-first test file that matches the twenty before it. Frontier tokens on zero-reasoning work.
- **The mechanism is pure routing.** Keep the frontier model for reasoning, intercept expensive file reads with **PreToolUse hooks**, route them to a cheap model (Gemini 2.5 Flash), and return only the relevant context. Two modes, no platform team.
- **The spend numbers are why this lands now.** A quarter of engineering leaders already burn **$200 to $500 per developer per month** on tokens, some are past **$2,000**, and AI coding cost is projected to pass the average developer's salary by **2028**.
- **The practitioner version of a result this wiki already tracks.** It is the same conclusion the handoff-tax measurement reached from the research side: what crosses a model boundary should be the minimum artifact, not the full context.

<div class="mz-links">

[Spotify Engineering](https://engineering.atspotify.com/2026/9/portal-by-spotify-cut-my-claude-code-token-usage-by-90) · [@dani_avila7](https://x.com/dani_avila7)
...
- **SigMap** grounds coding agents in repository structure with no LLM calls, embeddings or vector database, hitting the correct file in its top five **81.1% of the time against 44% for single-shot grep while cutting tokens 96.8%** across 21 repositories, and it can verify a plan line by line before the agent acts ([@agenticgirl](https://x.com/agenticgirl)).
- **OpenBMB open-sourced MiniCPM5-2B**, averaging 53.9 across 34 coding, math, long-context, tool-calling and agent benchmarks, with data, training recipe and RL stack released alongside the weights ([@di_zhang_fdu](https://x.com/di_zhang_fdu)).
- **Tencent's FlowBalance** ties dense self-guidance to the verifier's group advantage: keep the guidance when advantage is positive, reverse it when negative, shut it off when the rollout shows no preference. Reported faster convergence and better AIME24 diversity with no length collapse ([arXiv 2609.03241](https://arxiv.org/abs/2609.03241)).
- **"Stored Is Not Supported"** proposes provenance for persistent agent memory, keeping a claim's origin, evidence, dependencies, validity period and disclosure permissions separate. In a 24-case suite, trusting stored content released all 19 unsafe candidates while typed mediation released none unqualified ([@agenticgirl](https://x.com/agenticgirl)).
- **Evals belong at the center of harness engineering**, argues a guest essay: a harness gives an agent tools, memory, permissions and context, and evals are the only thing that tells you whether changing any of them helped ([Vanishing Gradients](https://hugobowne.substack.com/p/how-evals-are-central-to-harness)).
...
- **Schmidhuber pushes back on AGI claims**, arguing there is no AGI without mastery of the real world and no true self-improvement without self-improving *hardware*, as opposed to the meta-learning software that already exists ([@SchmidhuberAI](https://x.com/SchmidhuberAI)).
- **A useful practitioner counter-signal on Astra**, from someone who burned through Pro limits twice using it exclusively: the problem is not capability but that it "lacks taste" and is not opinionated, so it does whatever and wastes the budget ([@scaling01](https://x.com/scaling01)).
- **Skipped entirely:** Memgraph's product pitch, leaked-Meta-documents threads, Kurzweil's next bet, procrastination research, startup-idea lists, "new moats are old moats," an AI-engineer-is-a-career-trap thread, a brain-simulation-consciousness thread, and the assorted state-of-AI opinion reposts. No checkable claim between them.
