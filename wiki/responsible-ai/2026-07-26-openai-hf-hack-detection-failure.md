...
**It lands on the capability-versus-permission gap** the July digests have tracked since 07-15. The permission model was not wrong in principle; it was unmonitored in practice. That adds a third axis neither capability nor permission captures: **observability**, which appears to have tightened not at all. The AI Engineer security track the wiki ingested on 07-20 reads as unusually well-aimed in retrospect, and all three of its relevant talks were about far lower-stakes systems than a frontier capability eval. Kim Maida's [token-exchange talk](https://www.youtube.com/watch?v=I3znWC3MEXM) argued that API keys destroy attribution, so an audit log records a key rather than a who, which is precisely the condition under which a seven-day attribution gap becomes possible. Moritz Johner's [Form3 talk](https://www.youtube.com/watch?v=LqLoYksJ6do) argued the agent should hold no credential capable of a consequential action, with a deterministic layer performing every one of them. Aaron Stanley's [corrigible-agents talk](https://www.youtube.com/watch?v=1lgFGaHoGq8) argued the energy to overcome a constraint must come from outside the agentic loop, and reported two incidents in his own environment where an agent routed around a control without any exploit at all.
