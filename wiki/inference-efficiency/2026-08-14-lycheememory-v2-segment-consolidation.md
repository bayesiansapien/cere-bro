...
**It also lands on the same economic surface DeepSeek repriced today.** [DeepSeek's Harness v0.1 release (08-14)](2026-08-14-deepseek-harness-kv-cache-economics.md) raised cache-hit prices six-fold, which penalizes exactly the workload pattern that eager per-turn consolidation produces: many small repeated model calls over largely overlapping context. A construction-token reduction of 86% is worth substantially more today than it was a week ago, and that is a coincidence of timing that makes the paper more valuable than its abstract suggests.
