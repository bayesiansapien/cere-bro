...
## 2026-09-12: two arrivals say the parameter tensor carries structure the tooling throws away

**[FastE (09-12)](2026-09-12-faste-readout-triggered-token-compression.md) is the mirror image of [WRP (09-10)](2026-09-10-wrp-forward-free-depth-pruning.md) and the pair is worth stating as one claim.** WRP removes whole transformer blocks by estimating inter-layer redundancy directly from checkpoint weights, with no calibration data and no forward pass. FastE keeps every block and removes **tokens**, on a schedule keyed to depth, because prefix states in a final-readout embedding architecture are destructive to delete shallow and nearly free to delete deep (**40.11% fewer decoder FLOPs at 99.53% of nDCG@10** on NarrativeQA with Qwen3-Embedding-0.6B, training-free). **WRP says the deep layers are redundant with each other. FastE says the deep layers stop needing the input.** Two independent routes to the same conclusion, that the back half of an LLM does less independent work than its parameter count implies. **The composition is one experiment: prune with WRP, then re-measure FastE's depth-redundancy curve on the shortened stack.** If the curve flattens, both methods are cashing one budget.

**A third arrival makes the same kind of claim at training time, from the optimizer.** [Muon-C (09-12)](../llms-foundation-models/2026-09-12-muon-c-operator-aligned-optimizer.md) argues that unfolding a convolution kernel into a matrix, the standard way to apply Muon's orthogonalization, describes a **local patch map rather than the convolution operator**, so the geometry is wrong. Working in Fourier coordinates, where convolution is diagonal, and polarizing per-frequency channel-transfer blocks reaches **9.87 FID against 22.26 for unfolded Muon and 51.31 for Adam** on CIFAR-10 flow matching, and matches their final quality at **0.62x and 0.64x their applied FLOPs**. **A 38% training-FLOP reduction to equal quality is a compression result on an axis this page does not carry**: it reduces the cost of *reaching* a quality rather than the cost of running a fixed network, so it composes with everything here rather than competing. The caveat is large: CIFAR-scale only, and convolutions are not where frontier training spends its FLOPs.

---

## 2026-09-13: unstructured sparsity finally ships with its own kernels

**[Sparser, Faster, Lighter Transformer Language Models (09-13)](2026-09-13-sparser-faster-lighter-transformers.md)** (Sakana AI + NVIDIA, [code](https://github.com/SakanaAI/sparser-faster-llms)) is the first entry on this page that refuses the choice this page has organized itself around. Every prior result here has traded **quality against hardware friendliness**: unstructured pruning preserves accuracy and delivers no speedup, because irregular non-zero positions destroy the coalesced memory access and regular tiling that GPU matrix kernels depend on; structured pruning (2:4, block sparsity, mixture-of-experts) runs fast and costs accuracy.

**The inducement half is deliberately boring, which is the point.** Plain **L1 regularization** during training drives **over 99% unstructured sparsity in the feedforward layers with negligible downstream degradation**, backed by a quantitative sparsity study rather than one showcase model. No learned gate, no router, no hardware-shaped mask. Feedforward layers are the right target because they hold most of a model's parameters and most of its execution FLOPs, and the nonlinearity already zeroes most units on most tokens.

**The systems half is the contribution.** A new **sparse packing format** with three variants (aELL, bTwELL, and a hybrid that splits rows according to how their non-zeros distribute), each storing a values array, a tile-local index array, and per-tile non-zero counts, plus **CUDA kernels built to slot into the optimized execution pipelines modern GPUs already run**, for inference and training both. Reported benefits in throughput, energy and memory **increase with model scale**, which inverts how most efficiency techniques age.

**Three cross-page consequences.**

1. **It is a ninth axis for the [quantization page](quantization.md), at the individual-weight level.** That page's organizing claim has generalized from "precision should be allocated, not set" to "compute should be allocated, not set," with eight non-uniformity axes and **no two composed in any published system**. Unstructured feedforward sparsity is that claim at the finest possible granularity, and a 4-bit model at 99% sparsity is a cheap experiment with no published point.
2. **It manufactures the property that made [kimi-k3-in-c (09-12)](2026-09-12-kimi-k3-in-c-nvme-expert-streaming.md) possible.** That project ran a 2.78T-parameter mixture-of-experts model on one CPU in 8.24 GB by keeping 93% of the checkpoint on NVMe, and this wiki's reading was that **sparse activation is what makes cold bytes demotable, and a dense model has none.** L1 regularization creates cold bytes in a dense model. Induce the sparsity, then apply expert-streaming-style placement to the result. The two results are one day apart and nobody has composed them.
3. **It is the mechanism by which the [GPU kernels page's](../hardware/gpu-kernels.md) 09-13 argument gets acted on.** That page argued serving-cost differences between providers are increasingly kernel-quality differences, making vLLM, SGLang and TensorRT-LLM dispatch tables an outsized lever over the industry's inference bill. A jointly authored Sakana-NVIDIA open kernel release is exactly how such a lever gets pulled, and NVIDIA's presence on the author list cuts both ways.

**Caveats, and they are substantial.** This page's record is read from the paper's first page only: the "substantial throughput, energy and memory benefits" carry no speedup multiple, no hardware list and no accuracy table here. "Negligible impact on downstream performance" needs its benchmark suite before it means anything, since 99%-sparsity claims have historically been sensitive to which evaluations run. And **L1 is applied during training**, so this is not a post-training method: it requires training from scratch or a regularized fine-tune, at an unstated cost. Whether the sparsity pattern survives quantization or depth pruning is untested, which matters because [WRP (09-10)](2026-09-10-wrp-forward-free-depth-pruning.md) and [FastE (09-12)](2026-09-12-faste-readout-triggered-token-compression.md) both argue the back half of an LLM does less independent work than its parameter count implies, and they may be cashing the same budget.


---

## 2026-09-24: cross-layer sharing on the weight side

**[GeoPair](2026-09-24-geopair-cross-layer-factorization.md)** (training-free) optimizes which layers' projections pair up by structural compatibility rather than adjacency, then learns a shared dictionary per pair that preserves each layer's calibration geometry, plus structured sparsity. It is WRP's cross-layer redundancy claim (09-12) applied as parameter sharing instead of block deletion. Same day, [HySparse2](2026-09-24-hysparse2-two-level-kv-sharing.md) shared KV caches across layers with token-level sparse selection. Caveat: GeoPair's abstract carries no numbers, and shared dictionaries only cut latency with kernels that exploit them.


## 2026-09-25: pruning from the spec alone

**[Neural Spectral Capacity](2026-09-25-neural-spectral-capacity-calibration-free-pruning.md)** (2609.23087) scores an architecture from its layer shapes using the Marchenko-Pastur spectrum of random weight matrices, so it needs no weights, data or gradients. The score is layer-additive, so an exact dynamic program finds the budget-optimal allocation in seconds on a CPU. It prunes **LLaMA-7B to the best 5.7B across eight commonsense tasks with no calibration data, about 5,900x faster than the strongest training-free proxy**, and ranks near-equal-size FlexiBERT pairs at τ = 0.505 where parameter count gives 0.082. This is a new family on this page. Every prior structured pruner here (WRP 09-10, GeoPair 09-24, the SAE-guided methods of 08-31) needed calibration activations or trained weights. NSC decides *how much* each layer keeps, not *which* weights, so it composes with activation-aware pruners rather than replacing them. Open: comparison against Wanda or SparseGPT at equal size, and MoE expert-count selection. Same day, LLM Compressor v0.14 added distributed REAP expert pruning ([summary](2026-09-25-llm-compressor-v0-14-triton-gptq.md)).
