...
## 2026-09-08: a new task says the field over-buys depth and under-buys breadth, and the deletable-selection-layer pattern gains a setup-layer instance

**[Revision propagation (09-08)](2026-09-08-revision-propagation-test-time-compute.md)** (arxiv 2609.03254, Daisuke Kikuta, NTT, HuggingFace daily) benchmarks a task this page had no instance of: **propagating a local edit through every dependent part of an artifact built over a multi-turn conversation**, where the dependencies were established in dialogue and appear nowhere in the artifact itself. Holding the budget fixed and enumerating the available forms of test-time compute, it finds **parallel sampling with selection beats sequential self-revision and beats spending more reasoning tokens on one attempt, for 2.2 to 9.7 percentage points.**

**The mechanistic reading is the transferable part: dependency discovery is a recall problem, not a reasoning-depth problem.** Any single pass has some probability of overlooking a dependent, and thinking harder in one pass does not reliably fix an omission because the model does not know what it missed. Independent draws give union-like coverage at the recall step. **That predicts the gain should scale with the number of dependents rather than with reasoning difficulty**, which would explain the wide 2.2-to-9.7 range and is a claim the paper does not make.

**It is the second result in four days finding coverage, not depth, was the binding quantity.** [Locked at the Entrance (09-05)](../llms-foundation-models/2026-09-05-locked-at-the-entrance-rlvr-breadth.md) found that reinforcement learning with verifiable rewards destroys **up to 67% of a policy's solution coverage at the very first token**, and recovers 37% of it for free by interpolating with checkpoints already sitting on disk. Different subfield, same verdict: **the field buys sequential depth by default and the scarce quantity is breadth.** Given that most production reasoning budgets are spent on longer traces rather than more samples, that is a live allocation error and not a research curiosity.

**It also extends this page's discipline thread.** [Select, Compress, Reinvest (09-05)](2026-09-05-select-compress-reinvest-visual-tokens.md) established that compression pays nothing on its own and only earns accuracy when the freed budget is **reinvested** (eight query-selected frames beating sixteen uniformly spaced ones by 6.9 points), and CLEAR rations inference compute across a batch with a single shadow price. **This paper runs the same protocol on a new task: fix the budget, enumerate the forms of compute, measure which converts into accuracy.** That protocol keeps producing this page's most reusable findings and is still rare.

**One instance to watch: "selection" is unspecified, and this page has a pattern about that.** Picking among parallel candidates requires a selector, and the accumulating pattern here is that purpose-built learned selectors are frequently replaceable by something trivial. [Random Attention (09-04)](2026-09-04-random-attention-kv-eviction.md) deleted the KV importance scorer and matched the best evictor at 32-43% higher throughput; Select, Compress, Reinvest found a 1990s Orthogonal Matching Pursuit matching every purpose-built video frame selector; **[ACE (09-08)](2026-09-08-ace-expert-skipping-moe.md) adds a setup-layer instance by deleting the calibration set and the training step entirely** for MoE expert skipping, which is a stronger version of the pattern because a calibration-free method has no distribution-shift failure mode at all. Whether revision-propagation selection is majority voting, an LLM judge, or a trained scorer decides both its cost accounting and whether the result is robust, and the abstract does not say.

**Finally, a cost-allocation observation that belongs to the harness rather than the model.** The practical takeaway is that **for revision requests specifically, the marginal inference dollar should go to parallel candidates rather than a longer trace.** That is a per-turn test-time-compute policy switch conditioned on turn type, it sits in the [harness](../agentic-systems/agent-harness-engineering.md) rather than in the weights, and nothing ships it today.
...
## 2026-09-06: the process reward model may already be inside the policy

Every search-steering method on this page (best-of-N, step-level beam search, tree search, anything that grades intermediate steps) pays for a **process reward model**, a separately trained scorer that grades each reasoning step rather than only the final answer. That PRM is a second forward pass on every candidate step, and it is a standing serving-time tax this page has priced into nothing.

**[Sparse Reward Subsystem (09-06)](../responsible-ai/2026-09-06-sparse-reward-subsystem.md) (arXiv 2602.00986, Guowei Xu of Tsinghua with Mert Yuksekgonul and James Zou of Stanford) suggests the tax is avoidable.** Prior work established that LLM hidden states encode reward-related information but read it with black-box probes fitted on the full state, which shows the information is decodable and nothing about where it lives. This paper finds it **concentrated in a sparse subset of individual neurons**, splitting into two functional types: **value neurons**, whose activations predict state value (the expected probability that continuing from here yields a correct answer), and **dopamine neurons**, whose activations encode step-level temporal-difference error. Value neurons are reported robust and transferable across datasets and models with causal rather than correlational evidence, and the paper's own applications are the two this page cares about: value neurons as a confidence predictor, and **dopamine neurons as a process reward model guiding inference-time search.**

**If that holds, the PRM stops being a model you serve and becomes a view over a tensor you already computed.** Reading a sparse set of activation indices out of the policy's own forward pass costs a gather. That is the rare entry on this page where a deleted dependency is genuinely replaced by something close to free, against open problems 5 and 6, which both amount to the complaint that methods delete a serving-time dependency and never price the replacement.

**The number the paper owes and does not give in the abstract is the accuracy gap against a trained PRM.** Without it this is a cost claim with an unknown quality denominator, which is exactly the shape this page has criticized elsewhere.

**And there is a specific reason to expect the intrinsic PRM to inherit a known pathology.** [Locked at the Entrance (09-05)](../llms-foundation-models/2026-09-05-locked-at-the-entrance-rlvr-breadth.md) showed that reinforcement learning with verifiable rewards contracts a policy's solution coverage by up to 67%, with per-token likelihood shifts 11 to 16 times larger before the first operation than during downstream reasoning, so the damage is concentrated at the entrance. Value neurons exist, on this paper's own max-entropy-RL argument, **because the policy needs value estimates for its own decisions.** An RLVR-narrowed policy's value neurons are therefore calibrated to the narrowed distribution, so using them to steer search would steer toward the same entrance the training already selected. **That would make the intrinsic PRM cheaper and simultaneously more prone to the exact collapse the parallel-sampling literature exists to escape.** The experiment is a value-neuron sweep across an RLVR checkpoint sequence, and the checkpoints are already on disk, the same free-fix observation Locked at the Entrance made about late-layer interpolation.

**Fifth consecutive instance of the deletable-selection-layer pattern.** [Random Attention (09-04)](2026-09-04-random-attention-kv-eviction.md) deleted the KV-cache scorer and matched the strongest prior evictor at 32-43% higher vLLM throughput; [Select, Compress, Reinvest (09-05)](2026-09-05-select-compress-reinvest-visual-tokens.md) found unmodified Orthogonal Matching Pursuit from the early 1990s matching every purpose-built frame selector; [DRACO (09-05)](../agentic-systems/2026-09-05-draco-rubric-credit-assignment.md) beat its own verifier-trained baseline with closed-form credit redistribution; [AutoTraceGT (09-05)](../agentic-systems/2026-09-05-autotracegt-grounded-theory-agent-traces.md) recovered 73-91% of hand-built failure taxonomies with a 1967 qualitative coding method. **This adds the trained PRM to the list of learned components the system may already contain.** Five results, four days, five unrelated subfields, and the wiki should now treat "is the learned scorer load-bearing" as the default first question about any method that has one.

**Provenance note.** This paper reached the wiki through saved reading rather than the research feed, and it is not new: the arXiv identifier is 2602 and HuggingFace surfaced it on 2026-05-11. It is a four-month-old result recirculated by a viral post that also attributed it solely to Stanford and attached a 50%-reasoning-collapse ablation figure absent from the abstract. **Treat the mechanism as well-motivated and the headline numbers as unverified.**
