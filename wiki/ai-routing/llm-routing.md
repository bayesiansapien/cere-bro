...
**The physical axis arrives, and recovery becomes a routing decision (2026-05-31): hybrid cloud/device + Adaptive RePoT.** Two papers extend the routing surface in new directions. **When Cloud Agents Meet Device Agents** ([summary](2026-05-31-hybrid-cloud-device-agents.md)) studies hybrid multi-agent systems that split work across the device/cloud boundary, and adds the axis prior routing papers abstracted away: *edge energy*. It maps a three-way Pareto frontier (accuracy × monetary cost × edge energy), finds the optimal hybrid topology is highly task-dependent, and confirms the wiki's recurring lesson that more frontier compute is not monotonically better, escalating to the big model everywhere leaves the frontier. Crucially there is no learned router yet; the study characterizes the frontier but decides placement by ablation, leaving energy-aware routing as an open primitive. **Adaptive RePoT** ([summary](../agentic-systems/2026-05-31-repot-recoverable-program-of-thought.md)) is a trajectory-routing instance hiding in a reasoning paper: a rule-based dispatcher routes between *suffix repair* (resume from a verified checkpoint) and *fresh retry* based on verified-prefix length, because the right recovery move is capability-dependent (repair helps Gemini, ties GPT-medium/Claude, *hurts* GPT-mini). This is the same trajectory-level routing as Step-level Optimization (05-02), but the routed decision is a recovery strategy and the downside is real (wrong route can lower accuracy), making it a sharper test of trajectory routing than the upside-only escalation cascades.
