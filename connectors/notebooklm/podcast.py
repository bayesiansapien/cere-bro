...
def run(cmd: list[str], check: bool = True, auth_retries: int = 8) -> str:
    ...
    for attempt in range(max(1, auth_retries)):
        ...
        if transient and attempt < auth_retries - 1:
            ...
            # one so the retry of `cmd` picks up the freshened token. Verified:
            # upload succeeds ~attempt 4 with this warm-up, never without it.
...
def add_url_sources(urls: list[str]):
    # Add URLs one at a time so one bad URL does not drop the whole batch.
    for u in urls:
