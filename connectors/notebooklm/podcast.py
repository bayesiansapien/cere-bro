...
def run(cmd: list[str], check: bool = True, auth_retries: int = 5) -> str:
    ...
    can fail with "Authentication expired" and then self-heal on the very next
    call once the background refresh completes (verified: list attempt 1 fails,
    attempts 2-3 succeed). Since a weekly run makes dozens of nlm calls over
    10-20 min, any of them can hit this. So on an auth-expiry error we retry a
    few times with a short backoff instead of dying — only a persistent failure
    (token truly dead, needs `nlm login`) exits.
    """
    last = None
    for attempt in range(max(1, auth_retries)):
        ...
        # nlm's token flakiness surfaces two ways: "Authentication expired" on
        # the API calls and a bare "401 Unauthorized" on the resumable upload
        # endpoint (source add). Both self-heal on the next call after refresh.
